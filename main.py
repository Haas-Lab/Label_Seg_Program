...
def main():
    ...
    def morphology_transform(image, filter_method, radius, label_to_change):
        if filter_method == 'None':
            return image
        
        if filter_method == 'Erosion':
            if label_to_change != 0:
                mask = image == label_to_change
                mask = np.squeeze(mask,axis=0)
                eroded_mask = morphology.erosion(mask, morphology.ball(radius))
                output_image = image[0,...].copy()
                output_image[mask] = 0  # remove original label
                output_image[eroded_mask] = label_to_change  # assign eroded label
                return output_image
            else:
                # if no specific label is provided, erode the whole image
                image = np.squeeze(image, axis=0)
                return morphology.erosion(image, morphology.ball(radius))

        if filter_method == 'Dilation':
            if label_to_change != 0:
                mask = image == label_to_change
                mask = np.squeeze(mask,axis=0)
                expanded_mask = segmentation.expand_labels(mask, distance=radius)
                output_image = image[0,...].copy()
                output_image[mask] = 0  # remove original label
                output_image[expanded_mask] = label_to_change  # assign expanded label
                return output_image
            else:
                # if no specific label is provided, expand all labels
                image = np.squeeze(image, axis=0)
                return segmentation.expand_labels(image, distance=radius)
    ...
    @magicgui(
        ...
        label_to_change = {"widget_type": "SpinBox", 'max': 10, 'label': 'Label to transform'},
        ...
        if image is not None:
            ...
            if affected == 'neuron':
                ...
                if speckle_method != "None":
                    label_img = morphology_transform(label_img, speckle_method, radius, label_to_change)

                if small_object_count != 0:
                    label_img = to_categorical(label_img,num_classes=num_classes).astype(bool)
                    label_img[...,label_to_change] = morphology.remove_small_objects(label_img[...,label_to_change], min_size=small_object_count)
            ...
            if affected == 'noise':
                ...
                if speckle_method != "None":
                    label_img = morphology_transform(label_img, speckle_method, radius)
                label_img = morphology.label(label_img)
                label_img = morphology.remove_small_objects(label_img, min_size=small_object_count)
    ...
    def importPreviousLabels(image: ImageData, file_picker = 'N/A')-> LayerDataTuple:
        ...
        # load in the image and label and add to viewerq
